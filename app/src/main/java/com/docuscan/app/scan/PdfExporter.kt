...
object PdfExporter {
     ...
     * (FIT_TO_IMAGE matches the image's aspect ratio, fixed formats scale the image
     * to fit the chosen paper). The bitmap is downscaled to [quality]'s target DPI
     * and optionally converted to grayscale, mirroring makeacopy's PDF quality presets.
     */
    fun createPdf(
        pages: List<Bitmap>,
        out: OutputStream,
